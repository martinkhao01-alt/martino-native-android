...
class FormViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun save(
        nom: String, adresse: String, telephone: String, siteWeb: String,
        cuisine: String, notes: String, photoUri: String,
        notation: Float, prixFourchette: Int,
        onDone: (Long) -> Unit
    ) = viewModelScope.launch {
        val id = if (editId == 0L) {
            repo.insert(Restaurant(nom = nom, adresse = adresse, telephone = telephone,
                siteWeb = siteWeb, cuisine = cuisine, notes = notes,
                photoUri = photoUri, notation = notation, prixFourchette = prixFourchette))
        } else {
            repo.update(Restaurant(id = editId, nom = nom, adresse = adresse,
                telephone = telephone, siteWeb = siteWeb, cuisine = cuisine,
                notes = notes, photoUri = photoUri, notation = notation,
                prixFourchette = prixFourchette))
            editId
        }
        onDone(id)
    }
